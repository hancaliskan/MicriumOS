...
int main()
{
	...
	OSInit(&os_err);
		...
		OSTaskCreate((OS_TCB     *)&greenLedControllerTaskTCB,                /* Create the orange task                                    */
                 (CPU_CHAR   *)"Green Led controller",
                 (OS_TASK_PTR ) greenLedControllerTask,
                 (void       *) 0,
                 (OS_PRIO     ) 1,
                 (CPU_STK    *)&greenLedControllerTaskSTK[0],
                 (CPU_STK     ) 0u,
                 (CPU_STK_SIZE) 128u,
                 (OS_MSG_QTY  ) 0,
                 (OS_TICK     ) 0,
	 ...
	 while(DEF_TRUE)
	 {
		    ...
		    HAL_GPIO_TogglePin(GPIOD,GPIO_PIN_13);
				...
				if(_isSuspended)
				{
					...
					{
						OSTaskResume(&greenLedControllerTaskTCB, &os_err);
						resume_monitor = 0;
						_isSuspended = false;
					}
				}
